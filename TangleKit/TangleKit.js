...
Tangle.classes.TKNumberField = {
	...
	getValue: function () {
        var value = parseFloat(this.input.get("value").replace(',', ''));
...
Tangle.classes.TKAdjustableNumber = {
    ...
    initializeHelp: function () {
        ...
        this.helpElement.set("text", "drag left or right");
    },
...
Tangle.formats.twoDecimals = function (value) {
...
Tangle.formats.dollarsWithCommas = function (value) {
    x = value.round(2);
    new_string = "$" + x.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    if (value < 0) { return new_string.fontcolor("red"); }
    if (value > 1000000) { return new_string.fontcolor("red"); }
    return new_string;
};
